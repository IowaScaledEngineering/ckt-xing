...
ISR(TIMER0_COMPA_vect) 
{
	...
	static uint8_t centerLightPWMSetting = 0;
	...
	if (lightPWM < rightLightPWMSetting)
		tmp |= _BV(LAMP_RIGHT);
	if (lightPWM < centerLightPWMSetting)
		tmp |= _BV(LAMP_CENTER);

	LAMP_PORT = (LAMP_PORT & ~(_BV(LAMP_LEFT) | _BV(LAMP_RIGHT) | _BV(LAMP_CENTER))) | tmp;
...
bool audioIsPlaying()
{
	if (audioLoop || audioBufferDepth() > 0 || audioReadIdx < audioStartIdx + audioDataLen)
		return true;
	return false;
}
